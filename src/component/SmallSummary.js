...
return(
 ...
 key => <div>
  {size.filter((list)=>list.id == props.cart[key].itemid)
             .map((item)=>(
              <div>
   {product.filter((lists)=> lists.id == item.product_id) 
.map((items) => {
 total= item.price * props.cart[key].quantity;
 return(
    <div className="content22">
      <div >
            <div>
      <img className="checkimg2" src={require(`../assets/images/men/${items.image}`)} ></img>
      <p className="circle4"> {props.cart[key].quantity} </p>
     </div>
